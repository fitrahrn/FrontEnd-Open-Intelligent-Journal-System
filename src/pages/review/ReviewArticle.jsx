...
const ReviewArticle = () => {
    ...
    const getReviews = async () => {
        const response = await api.get(`https://backend-oijs-77pyv5kz2q-et.a.run.app/reviews/${article_id}`)
        ...
        setReview(listFile);
        const responseReviewer = await api.get(`https://backend-oijs-77pyv5kz2q-et.a.run.app/reviewers/user/${listFile.reviews_id}`)
        setReviewer(responseReviewer.data)
        const responseDiscussion = await api.get(`https://backend-oijs-77pyv5kz2q-et.a.run.app/discussion/${listFile.reviews_id}`)
    ...
    const ReviewArticle = async (e) => {
        ...
        try {
            await api.patch(`https://backend-oijs-77pyv5kz2q-et.a.run.app/reviewers`,formData, {
                "Content-type" : "multipart/form-data"
            });
            await api.post(`https://backend-oijs-77pyv5kz2q-et.a.run.app/email/certificate`, {
                    ...
                    <form className="content-container" onSubmit={ReviewArticle}>
                        ...
                        <div className="card border-light m-3 p-3">
                            <div className="row justify-content-end" >
                                ...
                                {!reviewer.recommendation ?<button type="submit" class="col-2 mx-2 btn btn-lg btn-primary">Submit</button>:<div></div>}
                            </div>
                        </div>    
                    </form>
                </div>
